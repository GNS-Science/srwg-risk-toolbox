...
from srwg_risk_toolbox.liquefaction.liquefaction_model import calc_pga_7pt5, SiteHazard, building_settlement_from_sources, ground_settlement_from_sources
...
def query_settlement_hazard(site, sc, liquefaction_parameters,building_settlement=True):

    site_apoes_hazard = query_geotechnical_hazard(site, sc)
    ...
    for rp in site_apoes_hazard.index:
        ...
        sg, _ = ground_settlement_from_sources(liquefaction_parameters)
        site_apoes_hazard.loc[rp, ['SG']] = sg
...
#### an alternate calculate_hcurve to toggle for logspace or not
# def calculate_hcurve(imt,hazard_table,imtl=None,logspace=True):
#
#     def extrapolate_linear_hcurve(return_periods, ims, imtl, kind='linear'):
#         x = ims
#         y = 1 / np.array(return_periods)
#         f = interp1d(x, y, bounds_error=False, kind=kind, fill_value='extrapolate')
#
#         hcurve = f(imtl)
#
...
#         hcurve = extrapolate_hcurve(return_periods, ims, imtl)
#     else:
#         hcurve = extrapolate_linear_hcurve(return_periods, ims, imtl)
#
#     return hcurve, imtl
...
def Vs30_to_Vs10(Vs30):
    """ Convert Vs30 values to Vs10

    Based on Eqn 5 and Table 2 in Boore (2004) Estimating Vs30
    log(Vs30) = a + b log(Vs(d))

    Args:
        Vs30: the average shear wave velocity of top 30m

    Returns:
        Vs10: the average shear wave velocity of top 10m
    """

    # coefficients for 10m from Table 2
    a = 0.042062
    b = 1.0292

    Vs10 = 10**((np.log10(Vs30) - a) / b)

    return Vs10


def Vs10_to_qc1Ncs(Vs10):
    """ Convert Vs10 values to q_c1Ncs

    based on Misko's suggested:

        q_c1Ncs = 1.39 x Vs(10) -170          (for 150 < Vs(10) < 250)
        qc1NCs = 50                           (for Vs(10) < 150
        not liquefiable if Vs(10) > 250 m/s)

    Args:
        Vs10: the average shear wave velocity of top 10m

    Returns:
        q_c1Ncs: corrected cone penetration test tip resistance
    """

    if Vs10 < 150:
        q_c1Ncs = 50
    elif Vs10 < 250:
        q_c1Ncs = 1.39 * Vs10 - 170
    else:
        q_c1Ncs = np.inf
